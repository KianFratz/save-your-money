...
- **Child ticket**: an issue linked to the map as a GitHub sub-issue. Where sub-issues are unavailable, use a task list and place `Part of #<map>` at the top of the child body. Labels use `wayfinder:<type>` (`research`, `prototype`, `grilling`, or `task`).
- **Blocking**: use GitHub's native issue dependencies. Where unavailable, use a `Blocked by: #<n>, #<n>` line.
- **Frontier query**: list the map's open children and drop tickets with an open blocker or assignee; first in map order wins.
